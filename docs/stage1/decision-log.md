...
5. 【2026-09-13 变更】任务按**模板工作单元**计费（训练按 step、推理按 batch/token/样本数，随模板版本锁定单价）；不按占用时长或固定报价收费。原「按硬件实测 GPU 运算量计费」口径经用户确认废止（变更记录见 D1）。
...
## D. 业务决策变更记录（用户拍板，2026-09-13）

### D1. 计费口径从「硬件实测运算量」改为「模板工作单元」（用户确认：选方案 B）

- **背景**：硬件性能计数（ncu）在全部可用环境受阻——WSL2 与 AutoDL 容器均实测 `ERR_NVGPUCTRPERM`（计数器开关属宿主内核参数，两种环境均无宿主控制权；见 gpu-metering-report.md §8）。原口径 C 的证据链无法闭合。
- **用户决策**：在三个候选（A 秒级 GPU 时长＝抄 vast/RunPod；B 模板工作单元；C 维持原案）中选择 **B**。
- **新口径定义**：
  - 每个模板声明其**计费工作单元**（训练模板：step（含前向+反向+优化）；批量推理模板：batch 或 token 或输出样本数，以模板计量字段为准）与**单价表**（随模板版本与订单锁定，金额用整数最小单位表示）。
  - 订单费用 C = Σ_u 单价_u × 实际完成单元数 u；多提供者按各自完成单元数分配，取整与余数规则待确认（原 E4/E5 仍适用）。
  - 单元数以**交付结果可复算**为核验口径：推理输出样本/token 可数、训练曲线含 step 数与固定 seed——核验组织能从结果独立重算（回应 spec §9.1 模板验收需可执行标准）。
- **废止/修订的 spec 条款**（已在 project-spec.md 带变更标注同步修订）：
  - §2「任务按硬件实测 GPU 运算量计费」→ 按 B 口径（变更 2026-09-13）。
  - §7.1/§7.2 计量要求与 Q_i/C 公式 → 改为工作单元口径；精度权重 w_p 表废止（不再区分 fp32/fp16 权重计费）。
  - §7.3 硬件计量实验 → 从计费前置条件降级为可选优化研究（已完成的实验结论保留）。
  - §2 作废清单不变：「按 GPU 占用时长直接收费」「按任务固定报价」仍作废——B 不是时长也不是整单固定价。
- **仍然有效的关联规则**：预算托管、整单验收、失败全退、费用全归提供者、平台不抽成——均不受本变更影响。
- **信任假设变化**：核验从「信任硬件计数器读数」转为「信任交付结果可复算 + 模板执行真实性」（伪造 step 数会在 loss 曲线/输出对账上暴露；比空跑刷时长的对抗面小）。详见 trust-assumptions.md §2 更新。
- **失效/改写的未决参数**：O1–O6 全部关闭（不再按精度计数计费）；M1 改写为「工作单元数的采信证据=结果复算」；M2 归因仅用于诊断；M4/R2 计量权限类问题不再阻塞计费；E1–E3 改写为「模板工作单元单价表」。已同步 undecided-parameters.md。
